...
class CreateTheory extends Component {
  constructor() {
    super();
    this.state = {
      idQuest: undefined,
    ...
    this.createTheory = this.createTheory.bind(this);
  ...
  createTheory() {
    const theory = {
	  ...
	  conteudo: this.state.value
    };
    api
      .post('/theory_manager', theory)
      .then((response) => {
        ...
        if (response.data.status) {
          ...
          this.showAlert('Conteúdo teórico cadastrado com sucesso.', 'success');
        } else {
          // console.log("res:", response.data.conceito)
          this.showAlert(
            'Não foi possível cadastrar o conteúdo teórico. Por favor, entre em contato com o administrador do sistema.',
            'error'
          );
        }
      })
			...
			<section className='jumbotron text-left item-hero'>
			  <div className='container'>
				<div className='row'>
				  <div className='col'>
					<React.Fragment>
					  <h1>Cadastrar Conteúdo Teórico</h1>
					  <h6>
						Possibilita associar um conteúdo teórico a um determinado conceito.
					  </h6>
					</React.Fragment>
				  </div>
				</div>
